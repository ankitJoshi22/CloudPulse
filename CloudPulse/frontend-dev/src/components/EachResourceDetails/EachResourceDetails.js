...
import { Container, Typography, Paper, Divider, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Button, Box } from "@mui/material";
...
const EachResourceDetails = () => {
  ...
  {data !== undefined &&<>
        <TableCell>{data.additionalAttributes?.id || "N/A"}</TableCell>
        <TableCell>{data?.displayName || "N/A"}</TableCell>
        <TableCell>{data?.assetType?.split('/')[1] || "N/A"}</TableCell>
        <TableCell>{data?.state || "N/A"}</TableCell>
        <TableCell>{data?.location || "N/A"}</TableCell>
        <TableCell>{data?.createTime || "N/A"}</TableCell>
        </>
  }
